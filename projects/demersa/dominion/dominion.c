...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case minion:
        minion_card(choice1, choice2, state, handPos);
        return 0;
...
int baron_card(int choice1, struct gameState *state, int handPos) {
	int currentPlayer = whoseTurn(state);
	...
	if (choice1 > 0) {	// boolean true or going to discard an estate
		...
		while (card_not_discarded) {
			if (state->hand[currentPlayer][p] == estate) {	// found an estate card
				state->coins += 4;	// add four coins to the amount of coins
...
int minion_card(int choice1, int choice2, struct gameState *state, int handPos) {
	int currentPlayer = whoseTurn(state);
	int i, j = 1;
...
int ambassador_card(int choice1, int choice2, struct gameState *state, int handPos) {
	int currentPlayer = whoseTurn(state);
	int i, j = 0;
	...
	// increase supply counts for chosen card by amount being discarded
...
int tribute_card(struct gameState *state, int handPos, int tributeRevealedCards[], int nextPlayer) {
	...
	for (i = 0; i <= 2; i++) {
		if (tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold) { 	//Treasure cards
			state->coins += 2;
		}

...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mineCard(choice1, choice2, handPos, currentPlayer, state);
    ...
    case minion:
        return minionCard(choice1, choice2, handPos, currentPlayer, state);
        /* //+1 action
        state->numActions++;
    ...
    case tribute:
        return tributeCard(currentPlayer, nextPlayer, state);
    ...
    case ambassador:
        return ambassadorCard(choice1, choice2, handPos, currentPlayer, state);
...
int baronCard(int choice1, int currentPlayer, struct gameState* state)
{
    state->numBuys++;//Increase buys by 1!
        if (choice1 > 0) { //Boolean true or going to discard an estate
            ...
            if (supplyCount(estate, state) > 0) {
                gainCard(province, state, 0, currentPlayer);//Gain an estate
...
int ambassadorCard(int choice1, int choice2, int handPos, int currentPlayer, struct gameState* state)
{
    int i = 0;
